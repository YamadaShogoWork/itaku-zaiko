...
| # | 対象画面 | 内容 | ステータス | 備考 |
|---|---|---|---|---|
| T-16 | 色マスタ | 重複チェックを input+debounce → blur に変更（ルール統一: サーバー通信は blur のみ） | 完了 | Client/Edit は元から blur。Color/Index のみ変更 |
| T-17 | 商品登録・編集 | 同名商品の登録をエラーにする（サーバー側: POST時にハードエラー、クライアント側: blur で警告表示） | 完了 | |
| T-18 | 取引先登録・編集 | POST時に同名取引先のサーバー側ハードエラーを追加（クライアント側 blur 警告は実装済み） | 完了 | |
| T-19 | ユーザー登録・編集 | blur で同名ユーザーの重複警告を追加（サーバー側ハードエラーは実装済み） | 完了 | |
