...
| # | 対象画面 | 内容 | ステータス | 備考 |
|---|---|---|---|---|
| T-1 | 色マスタ | 警告表示時に追加ボタンの高さがずれる問題を修正（警告をフォーム外に移動） | 未着手 | |
| T-2 | 色マスタ | 重複チェックのタイミングを blur → input イベントに変更（デバウンス 300ms） | 未着手 | |
| T-3 | 色マスタ | インライン編集フォームにも重複チェックを追加（currentId 除外） | 未着手 | |
| T-4 | 商品・取引先（共通） | 保存ボタン押下時に Fetch バリデーションを挟む。エラー時はダイアログを出さずエラーメッセージを表示 | 未着手 | ProductController・ClientController に Validate エンドポイント追加 |
| T-5 | 商品管理 | 掛け率入力の step="0.1" を step="any" に変更して HTML5 バリデーション警告を解消 | 未着手 | |
| T-6 | 商品管理 | 一覧の品名列グレー表示（上代・掛け率）を削除 | 未着手 | |
| T-7 | 商品管理 | 掛け率更新範囲ダイアログの「プレフィル値」を「初期値」など一般的な表現に変更 | 未着手 | |
| T-8 | 取引先管理 | 取扱商品テーブルの下代を初期表示から計算値（上代 × 掛け率）で表示 | 未着手 | 未チェック行も含む |
| T-9 | 取引先管理 | 掛け率列ヘッダーを左寄せ・rate-input の step を 0.1 に変更（novalidate 追加） | 未着手 | |
| T-10 | 取引先管理 | 削除ロジック変更：Delivery・SalesReport が 0 件なら ClientProducts ごと物理削除 | 未着手 | 仕様確認済み |
