...
// ダイアログハンドラを一元管理（残留ハンドラによる二重acceptを防ぐ）
let _acceptNext = false;
...
(async () => {
  ...
  await run('1-3', 'admin/admin でログイン→ダッシュボードへ遷移', async () => {
    ...
    await page.click('button[type="submit"]');
    // ログインページから離れればOK（waitForURL は URL オブジェクトを受けるので toString() が必要）
  ...
  await run('2-3', '色名空欄で追加→バリデーションエラー（保存されない）', async () => {
    ...
    // required属性によりブラウザバリデーションで止まるか、サーバー側でエラーになるはず
    // 行数が増えていなければOK
    ...
    for (const row of rows) {
      ...
      if (text.trim().startsWith('白') && !text.includes('黒')) {
        ...
        if (editBtn) {
          ...
          // inline-edit-form.active の input を操作
  ...
  await run('3-1', '「トートバッグ」（色なし）を登録', async () => {
    ...
    // 保存（handleSave → confirm → form submit）
    acceptNextDialog();
  ...
  await run('3-2', '「Tシャツ」（白・黒）を登録', async () => {
    ...
    await page.waitForTimeout(300);
    // 白・黒にチェック（name="SelectedColorIds"）
  ...
  await run('4-2', '取引先名が空欄のまま保存→バリデーションエラー', async () => {
    ...
    await page.waitForTimeout(1000);
    const url = page.url();
    // 一覧に戻っていなければ（バリデーションエラーで止まっていれば）OK
  ...
  await run('4-4', 'ABC商店の編集→「無効化する」→「有効化する」で戻す', async () => {
    // editリンクのhrefを取得（ToggleActive後にリストへリダイレクトされるため、editページURLを保持）
    const rows = await page.$$('table tbody tr');
    let editHref = null;
    for (const row of rows) {
    ...
    // 「無効化する」ボタンをクリック（confirm ダイアログを承認）
    const toggleBtn = page.locator('.status-section button[type="submit"]');
    await toggleBtn.scrollIntoViewIfNeeded();
    acceptNextDialog();
    await toggleBtn.click();
    // ToggleActive は /Client リストへリダイレクトする
    await page.waitForURL(/\/Client($|\?)/, { timeout: 10000 });
    await page.waitForLoadState('networkidle');

    // 再度 editページへ移動して無効化を確認
    await page.goto(BASE_URL + editHref);
    await page.waitForLoadState('networkidle');
    const body1 = await page.textContent('body');
    if (!body1.includes('無効') && !body1.includes('有効化する')) throw new Error('無効化されていない（editページ確認）');

    // 「有効化する」ボタンをクリック（こちらも confirm ダイアログあり）
    const toggleBtn2 = page.locator('.status-section button[type="submit"]');
